...
class System:
    ...
    def train_step(self, x, y):
        ...
        x_mse = torch.mean(F.mse_loss(y, F.interpolate(x.squeeze(0), scale_factor=4, mode='bicubic').unsqueeze(0)))
            ...
            for i in range(g_x.shape[1]):
                ...
                if mode == "testing_full":
                    if params["challenge"] == True:
                        ...
                        self.save_image(ix, "{}/{}_input.png".format(dir, str(idx * params["test sequence length"] + i).zfill(3)), normalize=True)
