...
class IntVIDFull(Dataset):
    def __init__(self, tst_idx=0):
        ...
        for i in range(params["test_startswith"], params["test_startswith"] + self.videos):
            self.file_list_input.append(sorted(glob.glob(root + "TestSource{}/".format(params['suffix']) + str(i).zfill(3) + "/*.png")))
            
            if params["challenge"] == False:
                self.file_list_target.append(sorted(glob.glob(root + "TestTarget{}/".format(params['suffix']) + str(i).zfill(3) + "/*.png")))
