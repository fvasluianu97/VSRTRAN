...
class HSA(nn.Module):
    def __init__(self, filter_size, nf, factor):
        ...
        self.expand_filter = expand_filter.repeat(nf, 1, 1, 1) # 128 is the channels of input hidden state features.

        self.nf = nf
        self.k = filter_size[2]
        self.factor = factor
        self.conv_filter = nn.Conv2d(3, num_filters, self.k, padding=1)

    def forward(self, h, i_t):
        filters = F.relu(self.conv_filter(i_t))
        ...
        input_expand = input_expand.view(B, self.nf, nF, H, W).permute(0, 3, 4, 1, 2) # [B, H, W, 128, k^2]
...
class SpatialAttentionModule(nn.Module):
    def __init__(self, k, num_filters, factor, activation=torch.sigmoid):
        ...
        self.key_conv_1 = nn.Conv2d(3, num_filters, self.k, padding=(self.k // 2))

        self.value_conv = nn.Conv2d(num_filters, num_filters, self.k, padding=(self.k // 2))
        self.query_conv = nn.Conv2d(num_filters, num_filters, self.k, padding=(self.k // 2))


    def forward(self, hs_t, i_t):
        k_it = F.relu(self.key_conv_1(torch.abs(i_t[:, 1] - i_t[:, 0])))
        qh_t = F.tanh(self.query_conv(hs_t))
        energy = k_it * qh_t 
        vh_t = self.activation(self.value_conv(hs_t))

        out = (torch.ones_like(vh_t).cuda() - vh_t) * hs_t + vh_t * energy 
        return out 
...
class SelfAttention(nn.Module):
    def __init__(self, k, num_filters, factor, activation=torch.sigmoid):
        ...
        self.input_conv = nn.Conv2d(3, num_filters, self.k, padding=(self.k // 2))
    ...
    def forward(self, hs_t, i_t):
        if params['riam'] == 'diff':
            ki_t = self.activation(self.input_conv(torch.abs(i_t[:, 1] - i_t[:, 0])))
        elif params['riam'] == 'max':
            ki_t = self.activation(self.input_conv(torch.maximum(i_t[:, 1], i_t[:, 0])))
        else:
            ki_t = self.activation(self.input_conv(i_t[:, 1]))
...
class UnbalancedTransformerCell(nn.Module):
    def __init__(self, num_blocks, kernel_size, filters_s, filters_d, factor):
        ...
        if params['att type'] == 'similarity':
            self.att_s = SpatialAttentionModule(kernel_size, filters_s, factor)
            self.att_d = SpatialAttentionModule(kernel_size, filters_d, factor)
        else:
            self.att_s = SelfAttention(kernel_size, filters_s, factor) # see later what activation can be used
...
class Transformer(nn.Module):
    def __init__(self, num_cells, attn_channels, state_dim, kernel_size, filters_s, filters_d, factor):
        ...
        self.gauss_kernel = get_gaussian_kernel(5, 4, 3) #543

        
        # downscaling
        self.conv1_s = nn.Conv2d(2*3 + filters_s + state_dim, filters_s, kernel_size, padding=int(kernel_size/2))
        self.conv1_d = nn.Conv2d(2*3 + filters_d + state_dim, filters_d, kernel_size, padding=int(kernel_size/2))
    ...
    def forward(self, x_in, state_in, fb_s, fb_d, mode='direct'):
        ...
        #current_state = torch.sigmoid(state_out) * self.conv_state(self.conv_norm_state(structure_x) + detail_x)
        ...
        s_out = self.conv_s_out(s) + F.interpolate(x_s[:, 1], scale_factor=4, mode='nearest')
        d_out = self.conv_d_out(d) + F.interpolate(x_d[:, 1], scale_factor=4, mode='nearest')

        out = self.conv_out(F.relu(self.conv_norm_out(s)) + d) + F.interpolate(x_in[:, 1], scale_factor=4, mode='nearest')
