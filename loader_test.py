...
class IntVID(Dataset):
    def __init__(self):
        ...
        for i in range(params["test_startswith"], params["test_startswith"] + self.videos):
            self.file_list_input.append(sorted(glob.glob(root + "TestSource{}/".format(params['bitrate']) + str(i).zfill(3) + "/*.png")))
            self.file_list_target.append(sorted(glob.glob(root + "TestTarget/" + str(i).zfill(3) + "/*.png")))
